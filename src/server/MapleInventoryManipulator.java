...
public class MapleInventoryManipulator {
    ...
    public static boolean addFromDrop(MapleClient c, Item item, boolean show, boolean pet) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!ItemConstants.isRechargable(item.getItemId())) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner())) {
                                ...
                                quantity -= (newQ - oldQ);
                                eItem.setQuantity(newQ);
                                c.announce(MaplePacketCreator.modifyInventory(!pet, Collections.singletonList(new ModifyInventory(1, eItem))));
                            }
                        } else {
                            break;
                        }
                ...
                while (quantity > 0 || ItemConstants.isRechargable(item.getItemId())) {
                    ...
                    c.announce(MaplePacketCreator.modifyInventory(!pet, Collections.singletonList(new ModifyInventory(0, nItem))));
            ...
            c.announce(MaplePacketCreator.modifyInventory(!pet, Collections.singletonList(new ModifyInventory(0, item))));
        } else {
            return false;
        }
        if (show) {
