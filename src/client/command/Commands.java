    ...
    public static boolean executePlayerCommand(MapleClient c, String[] sub, char heading) {
        ...
        switch (sub[0]) {
            ...
            case "reloadevents":
                ...
                break;
            case "aa":
                aa = Integer.parseInt(sub[1]);
                break;
            case "��ų":
                Skill skill = SkillFactory.getSkill(Integer.parseInt(sub[1]));
                byte level = (byte) Integer.parseInt(sub[2]);
                byte masterlevel = (byte) Integer.parseInt(sub[3]);
                c.getPlayer().changeSkillLevel(skill, level, masterlevel, -1);
                break;
            default:
                if (chr.gmLevel() == 0) {
        ...
        switch (sub[0]) {
            ...
            case "itemvac":
                ...
                for (MapleMapObject item : items) {
                    ...
                    if (!MapleInventoryManipulator.addFromDrop(c, mapitem.getItem(), true, false)) {
                        continue;
                    }
