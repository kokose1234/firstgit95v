    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (effect.getCooldown() > 0) {
            if (chr.skillisCooling(skillid)) {
                return;
            } else if (skillid != Captain.BATTLESHIP && !chr.isGM()) {
        ...
        if (skillid == Hero.MONSTER_MAGNET || skillid == Paladin.MONSTER_MAGNET || skillid == DarkKnight.MONSTER_MAGNET) { // Monster Magnet
            ...
            chr.getMap().broadcastMessage(c.getPlayer(), MaplePacketCreator.showBuffeffect(chr.getId(), skillid, chr.getLevel(), chr.getSkillLevel(skillid), direction), false);
            c.announce(MaplePacketCreator.enableActions());
            return;
        } else if (skillid == Viper.TIME_LEAP) { // Timeleap
            ...
            chr.removeAllCooldownsExcept(Viper.TIME_LEAP);
        } else if (skillid == Infighter.MP_RECOVERY) {// MP Recovery
        ...
        if (slea.available() == 5 || slea.available() == 6) {
