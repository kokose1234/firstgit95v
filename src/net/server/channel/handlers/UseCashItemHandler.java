        ...
        if (itemType == 505) { // AP/SP reset
            if (itemId > 5050000) {
                ...
                switch (APFrom) {
                    ...
                    default:
                        c.announce(MaplePacketCreator.updatePlayerStats(MaplePacketCreator.EMPTY_STATUPDATE, true, player.getJob().getId()));
                        return;
                }
                DistributeAPHandler.addStat(c, APTo);
                c.announce(MaplePacketCreator.updatePlayerStats(statupdate, true, player.getJob().getId()));
            }
            remove(c, itemId);
            ...
            for (byte i = 0; i < 3; i++) {
                ...
                if (pet != null) {
                    if (pet.canConsume(itemId)) {
                        ...
                        Item item = player.getInventory(MapleInventoryType.CASH).getItem(pet.getPosition());
                        player.forceUpdateItem(item);
                        player.getMap().broadcastMessage(c.getPlayer(), MaplePacketCreator.commandResponse(player.getId(), 1, true), true);
                        remove(c, itemId);
                        break;
                    }
                } else {
