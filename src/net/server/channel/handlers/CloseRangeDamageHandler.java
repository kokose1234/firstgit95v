    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (isFinisher(attack.skill)) {
            ...
            if (attack.skill != 1111008 && comboBuff != null) {
                ...
                int oid = player.isCygnus() ? SoulMaster.COMBO_ATTACK : Crusader.COMBO_ATTACK;
                int advcomboid = player.isCygnus() ? SoulMaster.ADVANCED_COMBO : Hero.ADVANCED_COMBO_ATTACK;
        ...
        if ((player.getSkillLevel(SkillFactory.getSkill(NightWalker.VANISH)) > 0 || player.getSkillLevel(SkillFactory.getSkill(WindBreaker.WIND_WALK)) > 0 || player.getSkillLevel(SkillFactory.getSkill(Rogue.DARK_SIGHT)) > 0) && player.getBuffedValue(MapleBuffStat.DARKSIGHT) != null) {// && player.getBuffSource(MapleBuffStat.DARKSIGHT) != 9101004
