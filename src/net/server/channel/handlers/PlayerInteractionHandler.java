    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
                    ...
                    if (shop.isOwner(c.getPlayer())) {
                        for (MaplePlayerShopItem mpsi : shop.getItems()) {
                            if (mpsi.getBundles() > 2) {
                                ...
                                MapleInventoryManipulator.addFromDrop(c, iItem, false, false);
                            } else if (mpsi.isExist()) {
                                MapleInventoryManipulator.addFromDrop(c, mpsi.getItem(), true, false);
                            }
                        }
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                ...
                MapleInventoryManipulator.addFromDrop(c, ivItem, false, false);
                c.announce(MaplePacketCreator.getPlayerShopItemUpdate(shop));
            }
            ...
            if (merchant != null && merchant.isOwner(c.getPlayer())) {
                ...
                if (item.getBundles() > 0) {
                    ...
                    MapleInventoryManipulator.addFromDrop(c, iitem, true, false);
                }
